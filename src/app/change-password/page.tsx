...
import { School, Lock, ShieldAlert, CheckCircle2, AlertCircle, ArrowRight, Loader2, Eye, EyeOff } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        setErrorMessage(data?.error?.message || 'Failed to update password.');
        setIsLoading(false);
        return;
      }
      ...
      setErrorMessage('Failed to update password due to a network error.');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {!isSuccess ? (
            <>
              <div className="text-center mb-6">
                <div className="w-10 h-10 rounded-xl bg-amber-50 border border-amber-100 text-amber-600 flex items-center justify-center mx-auto mb-2.5">
                  <ShieldAlert className="w-5 h-5" />
                </div>
                <h2 className="text-xl font-bold text-slate-900">Change Password Required</h2>
                <p className="text-xs text-slate-500 mt-1">
                  You are logged in with temporary credentials. You must set a new secure password before accessing the system.
                </p>
              </div>
            ...
            <div className="text-center py-6 space-y-4">
              ...
              <h2 className="text-xl font-bold text-slate-900">Password Changed Successfully</h2>
              <p className="text-xs text-slate-500 max-w-xs mx-auto leading-relaxed">
                Your temporary credentials have been updated. You can now log into your authorized portal.
              </p>
              <Link
                href="/login"
