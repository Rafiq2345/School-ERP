...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await LeaveEntitlementService.previewAnnualAllocation(tenantId, body);
    return NextResponse.json({ success: true, data: result });
  } catch (error: any) {
    return NextResponse.json(
      { success: false, error: { message: error.message || 'Failed to generate entitlement preview' } },
      { status: 400 }
    );
  }
}
