...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await LeaveEntitlementService.manualAdjustment(tenantId, body, userId);
    return NextResponse.json({ success: true, data: result });
  } catch (error: any) {
    return NextResponse.json(
      { success: false, error: { message: error.message || 'Failed to adjust leave balance' } },
      { status: 400 }
    );
  }
}
