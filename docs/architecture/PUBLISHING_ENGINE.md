...
When publishing in bulk (e.g., publishing 1,200 monthly fee vouchers or 800 result cards for Term 1):
1. **Phase 1: Dry-Run Preview**
   - Scans candidate records matching criteria (e.g., Class 9, Term 1).
   - Validates prerequisites:
     - All marks entered and verified?
     - Are any candidate vouchers zero-balance or flagged?
     - Are any records missing mandatory fields?
   - Returns a summary preview:
     - `Total Candidates`: 850
     - `Ready to Publish`: 842
     - `Blocked / Incomplete`: 8 (with itemized error list)
2. **Phase 2: Atomic Execution**
   - The user reviews the preview and confirms with their credentials/approval.
   ...
   - Dispatches background push/SMS notifications to parents/students if enabled.
...
```sql
-- Architectural Gating Invariant:
WHERE entity.publishing_status = 'PUBLISHED'
